...
if 'windows 2003' in os_name:
  ...
  if not dhcp:
    ...
    try:
      guest_command(serve, machine, 'cmd.exe', '/c echo for /f "skip=3 tokens=3*" %%a in (\'netsh int show int\') do netsh int ip set dns name="%%b" static {} primary & netsh int ip add dns name="%%b" {} index=2 > C:\\autovm.bat'.format(dns1, dns2))
    except:
      response(False, 62)
